...
def build_classifier(module_C, module_H, module_W, num_answers,
    ...
    if 'maxpool' in downsample or 'avgpool' in downsample:
        ...
        padding = 0 if ((module_H % pool_size == 0) and (
            module_W % pool_size == 0)) else 1
        layers.append(pool(kernel_size=pool_size,
                      stride=pool_size, padding=padding))
        prev_dim = proj_dim * \
            math.ceil(module_H / pool_size) * math.ceil(module_W / pool_size)
    if downsample == 'aggressive':
        layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
        layers.append(nn.AvgPool2d(
            kernel_size=module_H // 2, stride=module_W // 2))
        prev_dim = proj_dim
...
class FiLMedNet(nn.Module):
    def __init__(self, vocab, feature_dim=(1024, 14, 14),
        ...
        self.stem_use_coords = (stem_stride == 1) and (
            self.use_coords_freq > 0)
        ...
        self.default_weight = Variable(torch.ones(
            1, 1, self.module_dim)).type(torch.cuda.FloatTensor)
        self.default_bias = Variable(torch.zeros(
            1, 1, self.module_dim)).type(torch.cuda.FloatTensor)

        # Initialize stem
        stem_feature_dim = feature_dim[0] + \
            self.stem_use_coords * self.num_extra_channels
    ...
    def forward(self, x, film, save_activations=False):
        ...
        if self.condition_method == 'concat':
            ...
            cond_maps = cond_params.unsqueeze(3).unsqueeze(
                4).expand(cond_params.size() + x.size()[-2:])
        else:
        ...
        for fn_num in range(self.num_modules):
            ...
            if fn_num == (self.num_modules - 1):
                ...
                module_inputs_updated[:, fn_num +
                                      1] = module_inputs_updated[:, fn_num+1] + layer_output
                module_inputs = module_inputs_updated

        # Run the final classifier over the resultant, post-modulated features.
        if self.use_coords_freq > 0:
            final_module_output = torch.cat(
                [final_module_output, batch_coords], 1)
        if save_activations:
...
class FiLMedResBlock(nn.Module):
    def __init__(self, in_dim, out_dim=None, with_residual=True, with_batchnorm=True,
        ...
        if self.condition_method == 'conv-film' and self.with_cond[0]:
            self.film = FiLM()
        if self.with_batchnorm:
            self.bn1 = nn.BatchNorm2d(out_dim, affine=(
                (not self.with_cond[0]) or self.batchnorm_affine))
